...
import { buildSemanticMigrationPackagePrompt } from '../src/services/semanticMigration/prompts';
import {
...
test('legacy package prompt ignores confirmed plan prose instead of embedding its directives', () => {
  const inventory: MigrationInventory = {
    ...
    metrics: [],
    warnings: [],
    summary: 'No selected source objects.',
  };
  const rawPlan = 'PLAN ONLY. Do not return deployable YAML. This sentence must never reach compile.';
  const prompt = buildSemanticMigrationPackagePrompt({
    inventory,
    modelName: 'Target model',
    modelId: 'model-1',
    adminGoal: 'Compile only approved structured intent.',
    confirmedPlan: rawPlan,
  });

  assert.doesNotMatch(prompt, /PLAN\s+ONLY|Confirmed migration plan|This sentence must never reach compile/);
  assert.match(prompt, /Prior-stage prose is excluded/);
});

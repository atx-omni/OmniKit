...
test('BI Migration destination inventory requests exactly the two base model kinds', async () => {
  const calls: string[] = [];
  const records = {
    SHARED: [{ id: 'shared-model', kind: 'SHARED', name: 'Shared model' }],
    SHARED_EXTENSION: [{ id: 'extension-model', kind: 'SHARED_EXTENSION', name: 'Extension model' }],
  ...
  const inventory = await loadDestinationModelInventory(async (kind) => {
    calls.push(kind);
    return verifiedStudioModels(records[kind]);
  });

  assert.deepEqual(calls, [...DESTINATION_MODEL_KINDS]);
  assert.deepEqual(
    inventory.map((model) => [model.id, model.kind]),
    [
      ['shared-model', 'SHARED'],
  ...
  assert.equal(calls.includes('BRANCH'), false);
  assert.equal(calls.includes('QUERY'), false);
  assert.equal(calls.includes('WORKBOOK'), false);
});

test('BI Migration destination inventory merges a verified empty model kind', async () => {
  const inventory = await loadDestinationModelInventory(async (kind) => verifiedStudioModels(
    kind === 'SHARED' ? [{ id: 'shared-model', kind }] : [],
  ));
  assert.deepEqual(inventory, [{ id: 'shared-model', kind: 'SHARED' }]);
});

test('BI Migration destination inventory fails closed on unverified model collections', async () => {
  const cases: Array<{
    label: string;
    load: Parameters<typeof loadDestinationModelInventory>[0];
  }> = [
    {
    ...
    },
    {
      ...
      load: async (kind) => ({
        ...
        totalResults: kind === 'SHARED' ? 2 : 0,
        pageInfo: {
          ...verifiedStudioModels([]).pageInfo,
          totalRecords: kind === 'SHARED' ? 2 : 0,
        },
      }),
    },
    {
      label: 'wrong model kind',
      load: async (kind) => verifiedStudioModels(
        kind === 'SHARED' ? [{ id: 'wrong-kind', kind: 'QUERY' }] : [],
      ),
    },
    {
      label: 'duplicate model id across kinds',
      load: async (kind) => verifiedStudioModels([{ id: 'duplicate-model', kind }]),
    },
  ];

  for (const { label, load } of cases) {
    await assert.rejects(
      () => loadDestinationModelInventory(load),
...
test('BI Migration destination foundation resets tenant fields and invalidates stale provisioning', () => {
  const panel = source('src/components/semanticStudio/SemanticMigrationImportPanel.tsx');
  const connectionReset = panel.slice(
    panel.indexOf("setDestinationFoundationMode('existing_model')"),
    panel.indexOf('}, [connectionKey]);', panel.indexOf("setDestinationFoundationMode('existing_model')")),
  );
  for (const reset of [
  ...
  ]) assert.match(connectionReset, new RegExp(reset.replace(/[()]/g, '\\$&')));

  const modeChange = panel.slice(
    panel.indexOf('function changeDestinationFoundationMode'),
    panel.indexOf('async function handleProvisionDestinationFoundation'),
  );
  const foundationPanel = source('src/components/semanticStudio/MigrationDestinationFoundationPanel.tsx');
  assert.match(modeChange, /mode === destinationFoundationMode \|\| destinationFoundationProvisioning/);
  assert.match(modeChange, /destinationFoundationProvisionRequestRef\.current \+= 1/);
  assert.doesNotMatch(modeChange, /setDestinationFoundationProvisioning\(false\)/);
  assert.match(foundationPanel, /role="radio"[\s\S]{0,160}disabled=\{provisioning\}/);
  assert.match(
    foundationPanel,
    /'I confirm this shared model and connection are the approved destination\.',[\s\S]{0,100}!existingModelReady \|\| provisioning/,
  );
  assert.match(foundationPanel, /checked=\{checked\}[\s\S]{0,80}disabled=\{disabled\}/);
  assert.match(panel, /selectedConnectionId=\{verifiedDestinationFoundationConnectionId\}/);
  assert.match(panel, /connectionId: verifiedDestinationFoundationConnectionId/);
  assert.match(panel, /destinationFoundationMode === 'existing_connection' && !verifiedDestinationFoundationConnectionId/);
});

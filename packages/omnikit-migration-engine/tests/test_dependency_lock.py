...
def test_release_version_is_consistent_with_the_frozen_upstream_lock_and_provenance():
    ...
    frozen_upstream_project = next(
        package for package in uv_document["package"] if package["name"] == "omni-migrator"
    )

    assert __version__ == project_version
    assert frozen_upstream_project["version"] == project_version

...
export const walkthroughSteps: WalkthroughStep[] = [
  ...
  },
  {
    ...
    directions: [
      ...
      'When adding an optional external AI provider, choose its authentication method and follow the matching credential-creation guide. External provider credentials stay encrypted in the native vault, and the panel states exactly what OmniKit stores. Record the credential owner, expiration, and rotation date, then run Test. OpenAI and Anthropic use API keys; Snowflake and Databricks use the selected generated token; Omni AI needs no second secret or manual provider profile.',
      ...
      'For Snowflake Cortex, use a dedicated identity and least-privilege default role with Cortex REST access, then choose a PAT, OAuth access token, or generated key-pair JWT. OmniKit stores only that selected bearer value: never a password, OAuth client/refresh secret, private key, or key passphrase. Enter the account origin and Cortex model, record expiration, and run Test.',

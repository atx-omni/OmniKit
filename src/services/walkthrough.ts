export const WALKTHROUGH_VERSION = '2026-07-21-professional-looker-v2';
export const WALKTHROUGH_DISPLAY_VERSION = 'Updated July 21, 2026';
...
export const walkthroughSteps: WalkthroughStep[] = [
  ...
  },
  {
    ...
    directions: [
      ...
      'For Snowflake Cortex, use a dedicated identity and least-privilege default role with Cortex REST access, then choose a PAT, OAuth access token, or generated key-pair JWT. OmniKit stores only that selected bearer value: never a password, OAuth client/refresh secret, private key, or key passphrase. Select a model family that supports JSON-schema response formatting, record expiration, and run the exact structured-output Test.',
      'For a Databricks Foundation Model, grant the identity CAN QUERY on a chat-compatible Model Serving endpoint, enter the workspace origin and exact endpoint name, then run Test. OmniKit requires a READY endpoint and verifies the same structured-output invocation contract used during migration.',
      'For Databricks Genie, grant a dedicated identity access to the curated Genie Agent and backing SQL warehouse, copy the Agent ID (formerly Space ID), then generate an OAuth access token or a short-lived development PAT. OmniKit stores only that bearer value, not the service-principal client secret. Enter the workspace origin, record expiration, and run Test.',
      ...
      'For Domo, use Saved API for bounded Platform API inventory and add the optional server-only Product API token only when Deep Beast Mode and search evidence is required. Basic inventory remains available without it, but missing detail stays export-required rather than inferred.',
      ...
      'Treat Sigma as Preview while its deterministic engine remains in shadow. Use the Saved API path with the Sigma regional API URL, OAuth client ID, and client secret, or use one versioned offline API snapshot for diagnostic review. Review controls, formulas, lineage, grants, export schedules, materializations, input tables, writeback, and actions explicitly; generated SQL is evidence rather than source-query validation, and unsupported behavior requires a governed handoff.',

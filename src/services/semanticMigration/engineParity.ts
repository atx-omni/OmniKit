...
export const MIGRATION_ENGINE_SOURCE_POLICIES: Record<MigrationSourceTool, MigrationEngineSourcePolicy> = {
  ...
  looker: { source: 'looker', nativeAuthority: 'OmniKit raw LookML Manual Files parser plus compiled API evidence fallback', engineAuthority: 'OmniKit first-party raw LookML parsing and compiled scoped Looker API normalization', engineFormats: ['.model.lkml', '.view.lkml', '.dashboard.lookml', '.look.json'], nativeFormats: ['raw LookML and related files through guided upload'], rollback: 'Set the Looker engine mode to off or shadow.', owner: 'OmniKit' },
  ...
  sigma: { source: 'sigma', nativeAuthority: 'OmniKit discovery plus prepared-evidence API fallback', engineAuthority: 'OmniKit first-party Sigma Data Model spec and workbook-content normalization', engineFormats: ['versioned REST API evidence snapshot'], nativeFormats: ['versioned offline API evidence snapshot'], rollback: 'Set the Sigma engine mode to off or shadow.', owner: 'OmniKit' },

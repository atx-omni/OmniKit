...
export function DashboardMigrationWizard() {
  ...
  function duplicateActiveRouteGroup() {
    const duplicate: DashboardMigrationRouteGroupDraft = {
      ...
      permissionDecisionsByTargetId: Object.fromEntries(
        Object.entries(activeRouteGroup.permissionDecisionsByTargetId || {}).map(([targetRowId, decisions]) => [
          targetRowId,
  ...
  function setRouteGroupTargetAssignment(groupId: string, rowId: string, included: boolean) {
    ...
    setRouteGroups((current) => current.map((group) => {
      ...
      if (!included) delete permissionDecisionsByTargetId[rowId];
  ...
  function assignAllGroupsToAllDestinations() {
    ...
    setRouteGroups((current) => current.map((group) => ({
      ...
      permissionDecisionsByTargetId: Object.fromEntries(
        Object.entries(group.permissionDecisionsByTargetId || {}).filter(([targetRowId]) => targetRowIds.includes(targetRowId)),
      ),
      queryValidationWaiversByTargetId: Object.fromEntries(
